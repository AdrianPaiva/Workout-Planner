...
-(void)commonInitializer {
    ...
    //_table = [[UITableView alloc] initWithFrame:CGRectMake(0,0,320,460) style:UITableViewStylePlain];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (count == 0) {
        ...
        if ([indexPath row] == 1) {
            [[cell textLabel] setText:NSLocalizedString(@"No Exercises", @"A label for a table with no events.")];
        }
        else
        {
    ...
    NSNumber *weight = [[event info] objectForKey:@"weight"] ;
    NSString *weightString = [weight stringValue];
    
    NSNumber *reps = [[event info] objectForKey:@"reps"] ;
    NSString *repsString = [reps stringValue];
